...
public class CoralManipulatorSubsystem extends SubsystemBase {
  ...
  private static CoralManipulatorSubsystem instance = null; // static object that contains all movement controls
  ...
  public static CoralManipulatorSubsystem getInstance() {
    if (instance == null) {
      SubsystemChecker.subsystemConstructed(SubsystemType.CoralManipulatorSubsystem);
  ...
  public void startManipulatePercent(double leftPercentOutput, double rightPercentOutput){
    m_leftMotor.set(leftPercentOutput);
    m_rightMotor.set(rightPercentOutput);
  }

  public void startManipulateRPM(double leftRPM, double rightRPM)
  {
    //@todo: to add
  }
